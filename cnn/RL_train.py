...
def iterator(train_steps, valid_steps, save_filename, corruption):
    ...
    args = parser.parse_args(args=['--train_steps=%d' % train_steps, '--valid_steps=%d' % valid_steps, '--save', '--tune_all', '--tune_scales', '--dir=%s' % save_filename, '--entropy_weight=1e-3', '--log_interval=1'])
    # RL - Hyper-parameters default values for the warming train epochs 
    hyperparams = {}
    hyperparams['dropout0'] = args.start_drop
    hyperparams['dropout1'] = args.start_drop
    hyperparams['dropout2'] = args.start_drop
    hyperparams['dropout3'] = args.start_drop
    hyperparams['dropout4'] = args.start_drop
    hyperparams['fcdropout0'] = args.start_fcdrop
    hyperparams['fcdropout1'] = args.start_fcdrop
    hyperparams['indropout'] = args.start_indrop
    hyperparams['inscale'] = args.start_inscale
    hyperparams['hue'] = args.start_hue
    hyperparams['contrast'] = args.start_contrast
    hyperparams['sat'] = args.start_sat
    hyperparams['bright'] = args.start_bright
    hyperparams['cutlength'] = args.start_cutlength
    hyperparams['cutholes'] = args.start_cutholes
    ...
    def optimization_step(data_iter, data_loader, curr_epoch, hyper=False):
        ...
        global htensor, cnn, cnn_optimizer, hyperparams
        ...
        if not hyper:
            ...
            for key in hdict:
                if 'fcdropout' in key:
                    htensor_list.append(s_logit(torch.tensor(hyperparams[key]), min=0., max=0.9))
                elif 'dropout' in key:
                    htensor_list.append(s_logit(torch.tensor(hyperparams[key]), min=0., max=0.9))
                elif key in 'hue':
                    htensor_list.append(logit(torch.tensor(2*hyperparams[key])))
                elif key in ['contrast', 'sat', 'bright', 'indropout']:
                    htensor_list.append(logit(torch.tensor(hyperparams[key])))
                elif key in 'inscale':
                    htensor_list.append(s_logit(torch.tensor(hyperparams[key]), min=0, max=0.3))
                elif key in 'cutlength':
                    htensor_list.append(s_logit(torch.tensor(hyperparams[key]), min=0., max=25.))
                elif key in 'cutholes':
                    htensor_list.append(s_logit(torch.tensor(hyperparams[key]), min=0., max=4.))
    ...
    stats_to_yield = {'global_step' : [], 'dropout0' : [], 'dropout1' : [], 'dropout2' : [], 'dropout3' : [], 'dropout4' : [], \
                        'fcdropout0' : [], 'fcdropout1' : [], 'indropout' : [], \
                        'inscale' : [], 'hue' : [], 'contrast' : [], 'sat' : [], 'bright' : [], \
                        'cutlength' : [], 'cutholes' : [], \
    ...
    while train_epoch < args.warmup_epochs:
        ...
        if wup_step % args.log_interval == 0 and global_step > 0:
            ...
            stats_to_yield['val_acc'].append(summary_stats['acc'])
            stats_to_yield['val_loss'].append(summary_stats['loss'])
        ...
        while patience_elapsed < args.patience:
            ...
            if not hyper:
                ...
                if train_step % args.log_interval == 0 and global_step > 0:
                    ...
                    # RL - Saving of the train metrics and hyper-parameters values that will be presented to the agent
                    stats_to_yield['global_step'].append(global_step)
                    stats_to_yield['acc'].append(summary_stats['acc'])
                    stats_to_yield['loss'].append(summary_stats['loss'])
                    logger.write('train', summary_stats)
                    clear_stats(train_stats)
